...
__host__ __device__ int calculateBSDF(ray& r, staticGeom g, int closestIndex, glm::vec3 intersect, glm::vec3 normal, glm::vec3 emittedColor, AbsorptionAndScatteringProperties& currentAbsorptionAndScattering, glm::vec3& color, glm::vec3& unabsorbedColor, material m, material* mats, float xi1, float xi2, float xi3, int t)
{
	...
	//Choose reflective / refractive / diffuse based on the sampled probabilities
	int type = -1;
	...
	{
		if(RefractivityFactor <= 0.0f && ReflectivityFactor >= 0.0f)	//Handle the case of Both reflection and refraction being 0
		{
		...
		else if(RefractivityFactor > 0.0f && ReflectivityFactor <= 0.0f)
		{
		...
		{
			r.direction = getRandomDirectionInSphere(xi1, xi2);	//Generate Ray in any direction since it is recractive, reflective and diffuse
			r.origin = intersect + r.direction * 0.002f;
			r.keep = 1;
			color = m.color;
