...
//__global__ void GetRayCastFromCameraKernel(cameraData cam, float time, ray* InitialRays)
{
	...
	thrust::uniform_real_distribution<float> X1(-0.25, 0.25);
	float Xj = X1(rng);
	float Yj = X1(rng);
...
/*#ifndef USE_DEPTH_OF_FIELD	
	...
	glm::vec3 Mvec = cam.position + cam.view;	//Mvec
	glm::vec3 Hvec = (Avec * (float)(cam.view.length() * tan(glm::radians(cam.fov.x)))) / (float) Avec.length();	//Hvec
	glm::vec3 Vvec = (Bvec * (float)(cam.view.length() * tan(glm::radians(cam.fov.y)))) / (float) Bvec.length();	//Vvec
...
#else
	float w = X(rng);
	glm::vec3 tempCamPos = cam.position + glm::vec3(0.0, 0.0, w);
	
	glm::vec3 Avec = glm::cross(cam.view, cam.up);	//Avec
	glm::vec3 Bvec = glm::cross(Avec, cam.view);	//Bvec
	glm::vec3 Mvec = tempCamPos + cam.view;	//Mvec
	glm::vec3 Hvec = (Avec * (float)(cam.view.length() * tan(glm::radians(cam.fov.x)))) / (float) Avec.length();	//Hvec
	glm::vec3 Vvec = (Bvec * (float)(cam.view.length() * tan(glm::radians(cam.fov.y)))) / (float) Bvec.length();	//Vvec
...
//}
//
////Exclusive Scan Kernel Function
...
//		temp = InitialRays[index - (int)pow(2.0f, d-1)].newIndex + InitialRays[index].newIndex;
//	
//	__syncthreads();
//
//	InitialRays[index].newIndex = temp;
//}
//
...
void cudaRaytraceCore(uchar4* PBOpos, camera* renderCam, int frame, int iterations, material* materials, int numberOfMaterials, geom* geoms, int numberOfGeoms, bool &changed, float FocalDistance){
	...
	while(traceDepth <= MAX_DEPTH && numOfRays > 0)
	{
		...
		//Because stream compaction is slow, do it every 3rd depth
		if(traceDepth % 3 == 1)
		{
